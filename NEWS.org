...
This release updates the indent engine for matlab-ts-mode to electric indent code (aka pretty print
code or format code) by adjusting spacing around operators and all language elements. Electric
indent is controlled by the custom variable, ~matlab-ts-mode-electric-indent~ which defaults to ~t~.
