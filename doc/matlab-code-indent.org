...
2. How do I speed up a slow indent?
   ...
   When indent-mode=minimal is active, only the indent-level whitespace on left  is modified when
   the code is indented, which speeds up indent.

   It should be possible to speedup matlab-ts-mode electric indent if anyone is interested in trying
