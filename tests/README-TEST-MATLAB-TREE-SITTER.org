...
Check matlab tree-sitter indent by /sweeping/ over the =*.m= files in a directory tree and indenting
them. The indent sweep test can find issues in matlab-ts-mode *OR* in the tree sitter grammar.
...
1. Run sweep test
   ...
   Add option -save-diff to save a =*.diff= file:

   : /YOUR/WORK/DIRECTORY/Emacs-MATLAB-Mode/tests/sweep-test-matlab-ts-mode-indent.sh -save-diff
...
2. Results
   ...
   - *sweep-test-matlab-ts-mode-indent.diff*

     Indent differences. Only created if -save-diff was provided to
     sweep-test-matlab-ts-mode-indent.sh.

   - *sweep-test-matlab-ts-mode-indent.result.txt*

* Test: sweep-test-matlab-ts-mode-indent-no-mlint.sh

This is the same as [[Test: sweep-test-matlab-ts-mode-indent.sh]], but does not run mlint to validate
mlint output matches the matlab tree-sitter output and this is faster.

# LocalWords:  showall backports libtree SLIB dylib ABI abi MFILES utils VC vcvars logdone env
