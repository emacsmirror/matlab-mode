...
1. Install Emacs 30 or later and validate you emacs is on your system path and runs the desired version:
...
Check matlab tree-sitter parse by /sweeping/ over all *.m files in a directory tree.
...
2. Results
   ...
   - *sweep-test-matlab-ts-grammar.result.txt*
     ...
     This is created if matlab is available. If matlab is available, the codeIssues()
     matlab command is used to compare matlab parse v.s. the matlab tree-sitter parse
