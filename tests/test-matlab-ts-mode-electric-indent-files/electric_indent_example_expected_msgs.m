...
% t-utils-test-indent: no-line-by-line-indent - alignment of assignments not possible with line-by-line indent %  <{Matched rule: (matlab-ts-mode--i-top-level matlab-ts-mode--column-0 0)}>
...
yExp    = exp(x); %  <{Matched rule: (matlab-ts-mode--i-top-level matlab-ts-mode--column-0 0)}>
yTaylor = 1 + x + x.^2 / 2 + x.^3 / 6; % First few terms %  <{Matched rule: (matlab-ts-mode--i-top-level matlab-ts-mode--column-0 0)}>
...
plot(x, yExp, 'b-', 'LineWidth', 2); % Plot the actual exponential %  <{Matched rule: (matlab-ts-mode--i-top-level matlab-ts-mode--column-0 0)}>
hold on;                             % Keep the current plot %  <{Matched rule: (matlab-ts-mode--i-top-level matlab-ts-mode--column-0 0)}>
plot(x, yTaylor, ... %  <{Matched rule: (matlab-ts-mode--i-top-level matlab-ts-mode--column-0 0)}>
