...
[[https://www.mathworks.com/matlabcentral/answers/1881447-can-i-use-matlab-or-other-mathworks-products-with-windows-subsystem-for-linux-wsl-wsl2][MATLAB is not officially supported under wsl]], so MATLAB may not run correctly under wsl.  For
...
3. Install packages
   ...
   Notice we are installing emacs from Debian 12 backports to get Emacs 30.1
...
# LocalWords:  backports sudo shellcheck Debain ibverbs libasound libatomic libc libcairo gobject
# LocalWords:  libcap libcrypt libcups libdrm libfontconfig libfribidi libgbm libgdk pixbuf libgl
# LocalWords:  libglib libgstreamer libgtk libibverbs libice libltdl libnspr libnss libnuma libpam
# LocalWords:  libpango libpangocairo libpangoft libpixman libpsm librdmacm libsndfile libtirpc
# LocalWords:  libucx libuuid libwayland libxcomposite libxcursor libxdamage libxfixes libxfont
# LocalWords:  libxft libxinerama libxrandr libxt libxtst libxxf vm procps wget xkb utils zlib mw
# LocalWords:  mci bashrc esac libtree mnt setq magit flycheck melpa listp defun
