...
the behavior of font-lock without having to write lisp code to add the expected results of the
...
- Full re-evaluation of LANGUAGE-ts-mode.el
  ...
  each updated definition to re-evaluate them. Alternatively, using [[file:eval-buffer-fully.el]], you
...
1. Indent rules maybe easy to define using the treesit package pre-defined matchers and anchors
...
If the baseline doesn't exist or result doesn't match the baseline, the test fails and
...
`forward-sexp-default-function'."
  ...
  (let* ((move-back (and (numberp arg) (< arg 0)))
    ...
    (if (or match-paren
        ...
        ;; See tests/test-LANGUAGE-ts-mode-thing-settings-files/thing_forward_sexp1.m
...
#+begin_src emacs-lisp
  ...
  (ert-deftest test-LANGUAGE-ts-mode-fill-paragraph ()
    "Test fill paragraph using ./test-LANGUAGE-ts-mode-fill-paragraph-files/NAME.lang.
...
* News for matlab-ts-mode
  ...
  14. matlab-shell now has one correct ways of running code sections, via menu =MATLAB -> Code
  ...
  15. Builtin items such as namespaces, functions, classes, enumerations, etc. provided by the
  ...
  17. MLint MATLAB code analyzer integration via flycheck. Flycheck is an improved on-the-fly
  ...
  19. New "MATLAB -> View tree-sitter parse errors" menu item that lets one inspect
      the parse errors found by the MATLAB tree-sitter backing the new matlab-ts-mode.

  20. New "MATLAB -> Jump to function" menu item.

  21. New "MATLAB -> Check setup" menu item that helps validate flycheck and lsp-mode are
      setup correctly.

  22. New "MATLAB -> Editing -> ..." submenu that contains helpful editing commands.
