...
provide indentation (code formatting) and semantic coloring. The advantage of tree-sitter is that it
is faster, more accurate in context of syntax errors, and works without requiring the concept of a
project. You can open a source file from anywhere and tree-sitter can semantically color it, indent
it, etc.
...
LANGUAGE-ts-mode based on prog-mode which we recommend. Taking this approch eliminates unnecessary
...
single character. This makes it very easy to lock down the behavior of font-lock without having to
...
** prev-line for treesit-simple-indent-rules
  ...
  first blank line after the "b = 2;" statuement which may not be what you want. Prehaps prev-real
